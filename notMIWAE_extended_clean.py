...
class notMIWAE_extended:
    ...
    def __init__(self, X, Xval,
        ...
        # ---- parameters from regime encoder
        with tf.variable_scope('regime_encoder'):
            self.q_mu_u, self.q_log_sig2_u = self.regime_encoder(self.in_pl, self.s_pl)
        ...
        # --- evaluate the u-samples in q(u|x,s)
        ...
        self.log_q_u_given_x_s = tf.reduce_sum(q_u2.log_prob(self.l_u), axis=-1)
        ...
        tf.summary.scalar('Evaluation/quxs', tf.reduce_mean(self.log_q_u_given_x_s))
    ...
    #This is the encoder of u. In this verion, we take into account x_observed and s (no x_observed in second version)
    # We apply first layer separetly then we combine them

    def regime_encoder(self, x, s):

        h_data = keras.layers.Dense(units=self.n_hidden//2, activation=self.activation, name='l_regime_data')(x)
        h_mask = keras.layers.Dense(units=self.n_hidden//2, activation=self.activation, name='l_regime_mask')(s)
        
        concat = tf.concat([h_data, h_mask], axis=-1)
        h = keras.layers.Dense(units=self.n_hidden, activation=self.activation, name='l_regime_combine')(concat)
    ...
    def get_notMIWAE_extended(self, lpxz, lpsxu, lqzx, lquxs, lpz, lpu):

        # ---- importance weights
        l_w = lpxz + lpsxu + lpz + lpu - lqzx - lquxs
    ...
    def val_batch(self):
        ...
        for i in range(n_val_batches):
            ...
            _loss, _pxz, _psxu, _qzx, _quxs, _pz, _pu, _step = \
                self.sess.run([self.loss, self.log_p_x_given_z, self.log_p_s_given_x_u, 
                              self.log_q_z_given_x, self.log_q_u_given_x_s, 
        ...
        summary.value.add(tag="Evaluation/quxs", simple_value=quxs)
